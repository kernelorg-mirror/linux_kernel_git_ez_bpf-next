	...
	for (i = 0; i < nargs; i++) {
		...
		if (is_kfunc_arg_dynptr(meta->btf, arg)) {
			...
			if (reg->type == CONST_PTR_TO_DYNPTR) {
				...
				spi = dynptr_get_spi(env, reg);
				if (spi < 0) {
					verbose(env, "can't recognize dynptr param\n");
					return -EFAULT;
				}
...
struct inlinable_kfuncs_block {
	 ...
	 */
	const struct btf *btf;
	struct inlinable_kfunc funcs[8];
};
			    ...
			    struct inlinable_kfunc_regs_usage *usage)
{
	...
	for (i = 0; i < insn_num; ++i, ++insn) {
		...
		case BPF_ALU:
		case BPF_ALU64:
			if (BPF_SRC(insn->code) == BPF_X && insn->src_reg == BPF_REG_10)
				r10_escapes = true;
			break;
		default:
			break;
					  ...
					  u32 subprog)
{
	...
	for (i = BPF_REG_6; i <= BPF_REG_9; ++i)
		if (regs_usage.used_regs_mask & BIT(i))
			++extra_slots;
	*stack_depth_extra = extra_slots * BPF_REG_SIZE;
	insn_num = subprog_insn_cnt + extra_slots * 2;
	insn_buf = kmalloc_array(insn_num, sizeof(*insn_buf), GFP_KERNEL);
	if (!insn_buf)
		return ERR_PTR(-ENOMEM);

	verbose(env, "inlining kfunc %s at insn %d, insn_num=%d, used_regs=%02x, extra_slots=%d, subprog=%d, subprog_insn_cnt=%d, patch:\n",
		sh->name, insn_idx, sh->insn_num, regs_usage.used_regs_mask, extra_slots, subprog, subprog_insn_cnt);
	...
	for (i = 0; i < subprog_insn_cnt; ++i) {
		verbose(env, "patch %2d: ", i);
		print_bpf_insn(&cbs, insn_buf + i, env->allow_ptr_leaks);
	}
	...
	if (!regs_usage.r10_escapes && (regs_usage.used_regs_mask & BIT(BPF_REG_10)))
		stack_off = regs_usage.lowest_stack_off;

	for (i = insn_idx; i < insn_idx + insn_num; ++i) {
...
{
	struct bpf_subprog_info *info = env->subprog_info, *tmp;
	int cnt = env->subprog_cnt;
	struct bpf_prog *prog;

	if (env->subprog_cnt == env->subprog_cap) {
		env->subprog_cap *= 2;
		tmp = kvrealloc(env->subprog_info, env->subprog_cap + 1, GFP_KERNEL | __GFP_ZERO);
		if (!tmp)
			return -ENOMEM;
...
static int instantiate_inlinable_kfuncs(struct bpf_verifier_env *env)
{
	...
	for (i = 0; i < insn_cnt; ++i) {
		...
		kfree(insn_buf);
		if (err) {
			verbose(env, "can't instantiate inlinable kfunc %s at %i, err=%d\n",
				sh->name, i, err);
			return err;
		}
